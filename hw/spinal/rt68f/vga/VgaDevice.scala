...
case class VgaDevice(clk25: ClockDomain, disable: Boolean = true) extends Component {
  val io = new Bundle {
  ...
  if (disable) {
    // To disable the VGA, uncomment this section
    // and comment everything after the section.
    io.bus.DTACK := True
    io.bus.DATAI := 0
    io.vBlankInt := False
    io.vga.hSync := False
